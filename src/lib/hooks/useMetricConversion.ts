  ...
  useEffect(() => {
    if (
      ...
      if (heightMetric === "cm") {
        ...
        setConvertedHValue(
          `${height}cm/${(feet * 100).toFixed(1)}ft ${(
            remainingInches * 2.54
          ).toFixed(1)}in`
        );
      } else {
        ...
        const inches = parseInt(parts[1].replace("in", ""), 10);
        setConvertedHValue(
      ...
      if (weightMetric === "kg") {
        ...
        setConvertedWValue(
          `${weight}kg/${(convertedWeight * 2.20462).toFixed(1)}lb`
        );
      } else {
        // Convert weight from lbs to kg
        setConvertedWValue(
          `${(convertedWeight / 2.20462).toFixed(1)}kg/${weight}lb`
        );
      }
    }
